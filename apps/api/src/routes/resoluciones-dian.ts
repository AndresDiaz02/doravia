...
router.post("/", requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const nueva = await db.transaction(async (tx) => {
      ...
      // Si se indica consecutivo_inicial, el consecutivo_actual arranca en (consecutivo_inicial - 1)
      // para que el primer número emitido sea exactamente consecutivo_inicial.
      // Útil en migraciones desde otro proveedor DIAN (ej: venían en FE010, consecutivo_inicial=11 → primera = FE011).
      const inicialNum = consecutivo_inicial != null ? Number(consecutivo_inicial) : Number(consecutivo_desde);
      const actualInicial = Math.max(inicialNum - 1, Number(consecutivo_desde) - 1);
      ...
      const [row] = await tx
        ...
        .values({
          ...
          consecutivo_actual: actualInicial,
          consecutivo_inicial: inicialNum,
          fecha_desde,
          fecha_hasta,
    ...
    // Registrar en Plemsi automáticamente si el tenant tiene credenciales configuradas
    if (req.tenant.facturacion_electronica) {
      void (async () => {
        try {
