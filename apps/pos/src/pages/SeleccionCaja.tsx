      ...
      <div className="w-full max-w-2xl space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-base font-bold text-white">Selecciona una caja</p>
              <p className="text-xs text-slate-500">{user?.tenantNombre} · {user?.nombre}</p>
            </div>
          </div>
          <button onClick={logout} className="text-xs text-slate-600 hover:text-slate-300 transition-colors">
            Cerrar sesión
          </button>
        </div>

        {loading ? (
          <p className="text-center text-slate-600 text-sm py-8">Cargando cajas...</p>
        ) : cajas.length === 0 ? (
          <div className="rounded-2xl bg-slate-900 border border-slate-800 p-10 text-center">
            <Monitor className="h-12 w-12 text-slate-700 mx-auto mb-3" />
            <p className="text-slate-300 font-medium">No hay cajas configuradas</p>
            <p className="text-sm text-slate-600 mt-1">Un administrador debe crear las cajas desde el panel ERP.</p>
          </div>
        ) : (
            ...
            {cajas.map((caja) => {
              ...
              return (
                <button
                  ...
                  className="rounded-2xl bg-slate-900 border border-slate-800 p-5 text-left hover:border-violet-700/60 hover:bg-slate-800/60 transition-all group"
                >
                  <div className="flex items-start justify-between">
                    <div className="rounded-xl bg-slate-800 p-2.5 group-hover:bg-violet-900/40 transition-colors">
                      <Monitor className="h-5 w-5 text-slate-400 group-hover:text-violet-400 transition-colors" />
                    </div>
                    {turno && (
                      <span className="rounded-full bg-emerald-900/60 border border-emerald-700/50 px-2.5 py-0.5 text-xs font-medium text-emerald-400">
                        Turno abierto
                      </span>
                    )}
                  </div>
                  <p className="mt-3 text-base font-semibold text-white">{caja.nombre}</p>
                  {caja.descripcion && <p className="text-xs text-slate-500 mt-0.5">{caja.descripcion}</p>}
                  {turno && (
                    <p className="text-xs text-slate-500 mt-2">
                      Ventas del turno: <span className="font-semibold text-slate-300">{cop(turno.total_ventas)}</span>
                    </p>
                  )}
                  {!turno && (
                    <p className="flex items-center gap-1 text-xs text-violet-400 mt-2 font-medium">
        ...
        {cajaSeleccionada && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
            <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-sm space-y-4 shadow-2xl">
              <p className="text-lg font-semibold text-white">Abrir turno — {cajaSeleccionada.nombre}</p>
              <div className="space-y-1.5">
                <label className="text-xs text-slate-400 font-medium uppercase tracking-wide">Monto inicial en caja ($)</label>
                <input
                  type="number" min="0" step="1000"
                  value={montoInicial}
                  onChange={(e) => setMontoInicial(e.target.value)}
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-2xl font-bold text-center text-white focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500"
                  autoFocus
                />
              </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 rounded-xl border border-slate-700 bg-slate-800 py-3 text-sm font-medium text-slate-300 hover:bg-slate-700 transition-colors"
                >
                  Cancelar
                </button>
                <button
