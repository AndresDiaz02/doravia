    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
      <div className="mb-6">
        <div className="flex items-center gap-2">
          <div>
            <p className="mb-1 text-xs font-semibold uppercase tracking-[0.14em] text-action">Recaudo</p>
            <h1 className="text-2xl font-semibold tracking-tight text-gray-900 dark:text-gray-100">Cartera</h1>
          </div>
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Cuentas por cobrar, vencimientos y estado de cada cliente.</p>
      </div>
      ...
      {/* Tabs */}
      <div className="mb-6 flex w-full gap-1 overflow-x-auto rounded-xl border border-gray-200 bg-white p-1.5 dark:border-gray-700 dark:bg-slate-900">
        {(["resumen", "aging", "estado_cuenta"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`shrink-0 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
              tab === t ? "bg-action text-white shadow-sm shadow-action/20" : "text-gray-500 dark:text-gray-400 hover:bg-action/5 hover:text-gray-700 dark:hover:text-gray-200"
            }`}
          >
