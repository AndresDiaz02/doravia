...
interface Activity { id: string; opportunity_id: string | null; tipo: string; estado: string; scheduled_at: string; notas: string | null; }
...
export default function FundadorSales() {
  ...
  const load = useCallback(async () => {
    try {
      const [a, o, d, acts] = await Promise.all([apiFetch<Account[]>("/api/fundador/sales/accounts"), apiFetch<Opportunity[]>("/api/fundador/sales/opportunities"), apiFetch<Dashboard>("/api/fundador/sales/dashboard"), apiFetch<Activity[]>("/api/fundador/sales/activities?estado=pending")]);
      setAccounts(a); setOpportunities(o); setDashboard(d); setActivities(acts);
  ...
  async function completeActivity(activity: Activity) {
    try { await apiFetch(`/api/fundador/sales/activities/${activity.id}`, { method: "PATCH", body: JSON.stringify({ estado: "completed" }) }); await load(); }
    catch (err) { setError(err instanceof Error ? err.message : "No se pudo completar la actividad."); }
  }
  ...
  return <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
    ...
    <section><div className="flex items-center gap-2 mb-3"><CalendarDays className="h-5 w-5 text-slate-600"/><h2 className="font-semibold">Mi dia comercial</h2><span className="text-xs text-slate-500">Seguimientos pendientes y vencidos.</span></div><Card>{activities.length ? <div className="divide-y divide-slate-100">{activities.map((a) => <div key={a.id} className="flex gap-3 items-center px-4 py-3"><div className="flex-1"><p className="text-sm font-medium capitalize">{a.tipo.replace("_", " ")}</p><p className="text-xs text-slate-500">{a.notas || "Sin nota"} · {new Date(a.scheduled_at).toLocaleString("es-CO")}</p></div><Button size="sm" variant="secondary" onClick={() => void completeActivity(a)}>Completar</Button></div>)}</div> : <CardContent className="p-6 text-center text-sm text-slate-400">No hay actividades pendientes. Agenda el siguiente paso desde una oportunidad.</CardContent>}</Card></section>
  </div>;
}
