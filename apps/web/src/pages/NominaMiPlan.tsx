...
export default function NominaMiPlan() {
    ...
    return (
      <div className="p-6 max-w-2xl mx-auto">
        ...
        <Card className="p-8 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">Contacta a soporte para contratar un plan de nómina.</p>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 space-y-6 p-6 max-w-2xl mx-auto">
      <NominaBanner />
      <div>
        <h1 className="text-xl font-semibold text-gray-900">Mi plan de nómina</h1>
        <p className="text-sm text-gray-500">Estado de tu pool de documentos de nómina electrónica</p>
      </div>
      ...
      <Card className="p-5 space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900 mt-0.5">{PLAN_LABEL[pool.plan_slug] ?? pool.plan_slug}</p>
          </div>
      ...
      <Card className="p-5 space-y-3">
        <h2 className="text-sm font-semibold text-gray-700 flex items-center gap-1">
          <FileText className="h-4 w-4 text-gray-400" /> Documentos de este ciclo
        </h2>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2 text-sm">
          <div className="rounded-lg bg-gray-50 border border-gray-200 p-3">
            ...
            <p className="font-semibold text-gray-900">{pool.documentos_disponibles}</p>
          </div>
          <div className="rounded-lg bg-gray-50 border border-gray-200 p-3">
            <p className="text-xs text-gray-500">Acumulados de ciclos previos</p>
            <p className="font-semibold text-gray-900">{pool.documentos_acumulados_previos}</p>
          </div>
        </div>
        {pool.documentos_adicionales_comprados > 0 && (
          <p className="text-xs text-gray-500">+ {pool.documentos_adicionales_comprados} documentos adicionales comprados este ciclo</p>
        )}
