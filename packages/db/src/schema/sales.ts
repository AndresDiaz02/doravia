...
export const sales_quotes = pgTable("sales_quotes", {
  ...
  valid_until: date("valid_until"), status: varchar("status", { length: 30 }).$type<(typeof SALES_QUOTE_STATUSES)[number]>().notNull().default("draft"), terms: text("terms"), notes: text("notes"), public_token: varchar("public_token", { length: 96 }).notNull().unique(), version: integer("version").notNull().default(1),
