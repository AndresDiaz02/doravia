...
async function limpiarTenants(preservar: string[]) {
  ...
  await db.execute(sql`DELETE FROM lineas_asiento WHERE asiento_id IN (SELECT id FROM asientos_contables WHERE tenant_id = ANY(${eliminar}))`);
  await db.execute(sql`DELETE FROM asientos_contables WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM retenciones_factura WHERE factura_id IN (SELECT id FROM facturas WHERE tenant_id = ANY(${eliminar}))`);
  await db.execute(sql`DELETE FROM items_nota_credito WHERE nota_credito_id IN (SELECT id FROM notas_credito WHERE tenant_id = ANY(${eliminar}))`);
  await db.execute(sql`DELETE FROM notas_credito WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM items_factura WHERE factura_id IN (SELECT id FROM facturas WHERE tenant_id = ANY(${eliminar}))`);
  await db.execute(sql`DELETE FROM facturas WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM items_cotizacion WHERE cotizacion_id IN (SELECT id FROM cotizaciones WHERE tenant_id = ANY(${eliminar}))`);
  await db.execute(sql`DELETE FROM cotizaciones WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM items_venta_pos WHERE venta_id IN (SELECT id FROM ventas_pos WHERE tenant_id = ANY(${eliminar}))`);
  await db.execute(sql`DELETE FROM ventas_pos WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM abonos_fiado WHERE fiado_id IN (SELECT id FROM fiados WHERE tenant_id = ANY(${eliminar}))`);
  await db.execute(sql`DELETE FROM items_fiado WHERE fiado_id IN (SELECT id FROM fiados WHERE tenant_id = ANY(${eliminar}))`);
  await db.execute(sql`DELETE FROM fiados WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM citas_pos WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM turnos_pos WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM cajas_pos WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM componentes_producto WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM movimientos_inventario WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM productos WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM bodegas WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM clientes WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM resoluciones_dian WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM retenciones_config WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM cuentas_contables WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM centros_costos WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM tutorial_progress WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM retencion_seguimiento WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM pending_registrations WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM plantillas_factura WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM gastos WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM proveedores WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM audit_log WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM user_accesos WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM comisiones_contador WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM refresh_tokens WHERE user_id IN (SELECT id FROM users WHERE tenant_id = ANY(${eliminar}))`);
  await db.execute(sql`DELETE FROM password_reset_tokens WHERE user_id IN (SELECT id FROM users WHERE tenant_id = ANY(${eliminar}))`);
  await db.execute(sql`DELETE FROM users WHERE tenant_id = ANY(${eliminar})`);
  await db.execute(sql`DELETE FROM tenants WHERE id = ANY(${eliminar})`);
